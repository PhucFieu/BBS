                    ...
                    <script>
                        document.addEventListener('DOMContentLoaded', function () {
                            ...
                            departureCitySelect.addEventListener('change', function () {
                                ...
                                updateRouteName(); // Auto-fill route name
                            });
                            ...
                            // Auto-fill Route Name based on selected cities
                            function updateRouteName() {
                                const depCityId = departureCitySelect.value;
                                const destCityId = destinationCitySelect.value;
                                const routeNameInput = document.getElementById('routeName');

                                // Only auto-fill if both cities are selected and route name is empty or follows the auto-generated pattern
                                if (depCityId && destCityId && routeNameInput) {
                                    const depCity = citiesData[depCityId];
                                    const destCity = citiesData[destCityId];

                                    if (depCity && destCity) {
                                        const currentValue = routeNameInput.value.trim();
                                        const isAutoGenerated = !currentValue ||
                                            currentValue.includes(' - ') &&
                                            Object.values(citiesData).some(c => currentValue.startsWith(c.name));

                                        // Auto-fill if empty or if it looks like an auto-generated name
                                        if (isAutoGenerated || !currentValue) {
                                            routeNameInput.value = depCity.name + ' - ' + destCity.name;
                                        }
                                    }
                                }
                            }
