...
<body>
    ...
    <div class="container mt-4">
        ...
        <div class="search-box">
            ...
            <form method="get" action="${pageContext.request.contextPath}/staff/tickets" class="row g-3">
                <div class="col-md-5">
                    <input type="text" name="search" class="form-control" 
                           placeholder="Search by ticket number, phone, or name" 
                           value="${searchTerm}">
                </div>
                <div class="col-md-4">
                        ...
                        <tbody>
                            <c:forEach var="ticket" items="${tickets}">
                                <tr class="ticket-row">
                                    ...
                                    <td><span class="badge bg-secondary">${ticket.seatNumber}</span></td>
                                    <td>
                                        <span class="badge ${ticket.status == 'CONFIRMED' ? 'bg-success' : 
                                            ticket.status == 'CHECKED_IN' ? 'bg-info' : 
                                            ticket.status == 'CANCELLED' ? 'bg-danger' : 'bg-warning'}">
                                            ${ticket.status}
                                        </span>
                                    </td>
                                    <td>
                                        <span class="badge ${ticket.paymentStatus == 'PAID' ? 'bg-success' : 'bg-warning'}">
                                    ...
                                    <td>
                                        ...
                                        <c:if test="${ticket.paymentStatus != 'PAID' && ticket.status != 'CANCELLED'}">
                                            <form method="post" action="${pageContext.request.contextPath}/staff/mark-paid" style="display: inline;">
                                                <input type="hidden" name="ticketId" value="${ticket.ticketId}">
                                                <input type="hidden" name="redirectTo" value="${pageContext.request.contextPath}/staff/tickets?search=${searchTerm}&date=${searchDate}">
